...
from model import Esm2BartSeq2Seq  # Import the model from model.py
...
def transform_sequence_and_structure_by_esm2bart(seq: typing.Optional[typing.Union[str, typing.List[str]]],
                                                 esm2bart_model: Esm2BartSeq2Seq,
                                                 tokenizer_in, tokenizer_out) -> torch.Tensor:
    def generate_hash_id(value):
        return hashlib.sha256(value.encode()).hexdigest()

    def process_single_sequence(s):
        # Tokenize input sequence using the input tokenizer of the Esm2 model
        input_ids = tokenizer_in(s, return_tensors="pt").input_ids.cuda()
        # Tokenize the labels (for supervised learning or generation)
        labels = tokenizer_out(s, return_tensors="pt").input_ids.cuda()
        # Forward pass through Esm2BartSeq2Seq
        with torch.no_grad():
            _, logits = esm2bart_model(input_ids=input_ids, labels=labels)
        return logits  # Return the logits as embeddings
...
def load_esm2bart() -> Esm2BartSeq2Seq:
    # Instantiate the Esm2BartSeq2Seq model
    ...
    with torch.no_grad():
        with tqdm(test_dataloader, unit='batch') as tepoch:
            for batch, data in enumerate(tepoch):
                ...
                SH_encoded = transform_sequence_and_structure_by_esm2bart(seq=SH, esm2bart_model=esm2bart_model, tokenizer_in=tokenizer_in, tokenizer_out=tokenizer_out)
                SV_encoded = transform_sequence_and_structure_by_esm2bart(seq=SV, esm2bart_model=esm2bart_model, tokenizer_in=tokenizer_in, tokenizer_out=tokenizer_out)
                # #################################
...
def train_pretrain(train_dataloader, model: TMO_Net, esm2bart_model: Esm2BartSeq2Seq, epoch, optimizer, dsc_optimizer, omics: typing.Dict, fold, tokenizer_in, tokenizer_out):
    model.train()
    ...
    with tqdm(train_dataloader, unit='batch') as tepoch:
        for batch, data in enumerate(tepoch):
            ...
            # Encode SH and SV using the Esm2BartSeq2Seq model
            SH_encoded = transform_sequence_and_structure_by_esm2bart(seq=SH, esm2bart_model=esm2bart_model, tokenizer_in=tokenizer_in, tokenizer_out=tokenizer_out)
            SV_encoded = transform_sequence_and_structure_by_esm2bart(seq=SV, esm2bart_model=esm2bart_model, tokenizer_in=tokenizer_in, tokenizer_out=tokenizer_out)
...
def SH_SV_Dataset_pretrain(train_dataloader, test_dataloader, epochs, device, omics: typing.Dict, fold, file_type=""):
    # model init
    model = TMO_Net(4, [320, 320, 1200, 1200], 64, [320, 128, 64],
...
def start_pretrain():
    ...
    # Initialize Esm2BartSeq2Seq and tokenizers
    esm2bart_model = load_esm2bart()
