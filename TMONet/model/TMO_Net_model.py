...
from TMONet.util.loss_function import KL_loss, reconstruction_loss, KL_divergence
...
class encoder(nn.Module):
    def __init__(self, input_dim, latent_dim, hidden_dim):
        ...
        self.encoder = nn.Sequential(nn.Linear(input_dim, hidden_dim[0]),
                                     nn.InstanceNorm1d(hidden_dim[0]),
                                     nn.ReLU(),

                                     nn.Linear(hidden_dim[0], hidden_dim[1]),
                                     nn.InstanceNorm1d(hidden_dim[1]),
                                     nn.ReLU(),
    ...
    def reparameterize(mean, logvar):
        ...
        epsilon = torch.randn_like(std)  # 根据 std
        return epsilon * std + mean
...
class TMO_Net(nn.Module):
    def __init__(self, modal_num, modal_dim, latent_dim, hidden_dim, omics_data, pretrain=False):
        ...
        #   modality-invariant representation 64 -> 64
    ...
    def compute_generate_loss(self, input_x, batch_size, omics):
        ...
        cross_elbo, cross_infer_dsc_loss, pearson_scores, input_data, reconstruct_data = self.cross_elbo(output,
                                                                                                         input_x,
                                                                                                         batch_size,
                                                                                                         omics, mask_k)
    ...
    def compute_dsc_loss(self, input_x, batch_size, omics):
        ...
        # cross_elbo + cross_infer_loss + 0.01 * cross_modal_KL_loss, cross_modal_dsc_loss
        dsc_loss = self.adversarial_loss(batch_size, output, omics, mask_k)
    ...
    def share_representation(self, output, omics):
        ...
        return share_features  # [64,64,64,64]
    ...
    def self_elbo(self, input_x, input_omic, omics, mask_k):
        ...
        for item, i in enumerate(values):
            if i != mask_k:
                ...
                self_vae_elbo += 0.01 * KL_loss(mu, log_var, 1.0) + reconstruction_loss(input_omic[item],
                                                                                        reconstruct_omic, 1.0,
                                                                                        self.omics_data[i])
    ...
    def cross_elbo(self, input_x, input_omic, batch_size, omics, mask_k):
        ...
        for i in range(self.k):
            if i in values:
                ...
                reconstruct_data.append(reconstruct_omic.detach().cpu().numpy())
                input_data.append(input_omic[values.index(i)].detach().cpu().numpy())

                # 计算重建损失和KL散度
                cross_elbo += 0.01 * KL_loss(poe_mu, poe_log_var, 1.0) + reconstruction_loss(
                    input_omic[values.index(i)], reconstruct_omic, 1.0,
                    self.omics_data[i])
                ...
                # 计算Pearson相关系数
                pearson_score = \
                    torch.corrcoef(torch.stack([input_omic[values.index(i)].flatten(), reconstruct_omic.flatten()]))[
                        0, 1]
        ...
        return (cross_elbo + cross_infer_loss + 0.01 * cross_modal_KL_loss, cross_modal_dsc_loss,
                pearson_scores, input_data, reconstruct_data)
    ...
    def get_embedding(self, input_x, batch_size, omics):
        output, share_representation = self.forward(input_x, omics)  # input_x: [256,256,1200,1200]
        # todo 二维矩阵的短方框元素 output
        # latent_z, mu, log_var = output[0][0]
        torch.save(output, f'./model/model_embedding/SH_SV_embedding_output_matrix.pt')
        embedding_tensor = []
        ...
        for i in range(self.k):
            ...
            for j in range(self.k):
                if (i != j) and (j in values):
                    ...
                    mu_set.append(mu)
                    log_var_set.append(log_var)
            poe_mu, poe_log_var = product_of_experts(mu_set, log_var_set)  # 除对角线之外
            if i in values:
                _, omic_mu, omic_log_var = output[i][i]
                joint_mu = (omic_mu + poe_mu) / 2
            else:
                joint_mu = poe_mu
            embedding_tensor.append(joint_mu)  # 64 维度向量
        embedding_tensor = torch.cat(embedding_tensor, dim=1)  # 【4，64】
        # todo 长方框 embedding：embedding_tensor
        torch.save(embedding_tensor, f'./model/model_embedding/SH_SV_embedding_tensor_matrix.pt')
...
class DualAutoEncoder(nn.Module):
    ...
    def forward(self, x_a, x_b):
        ...
        recon_a_loss = reconstruction_loss(x_a, recon_a_from_a, 1, 'gaussian') + reconstruction_loss(x_a,
                                                                                                     recon_a_from_b, 1,
                                                                                                     'gaussian')
        recon_b_loss = reconstruction_loss(x_b, recon_b_from_b, 1, 'gaussian') + reconstruction_loss(x_b,
                                                                                                     recon_b_from_a, 1,
                                                                                                     'gaussian')
...
if __name__ == '__main__':
    ...
    model = DownStream_predictor(4, [6016, 6617, 4539, 7460], 64, [2048, 1024, 512],
                                 pretrain_model_path, tasks['ERStatus'], omics, fixed, omics)
    # print(model)

    # 定义每种模态的数据维度
    modal_dims = [6016, 6617, 4539, 7460]  # 给出的特征数
    batch_size = 1  # 假设每个批次有10个样本

    # 根据 omics 字典的顺序生成虚拟数据，确保输入数据与模型期待的结构相匹配
    omics = {'gex': 0, 'methy': 1, 'mut': 2, 'cna': 3}
    input_x = [
        torch.randn(batch_size, modal_dims[omics['gex']]),  # 基因表达数据
        torch.randn(batch_size, modal_dims[omics['methy']]),  # 甲基化数据
        torch.randn(batch_size, modal_dims[omics['mut']]),  # 突变数据
        torch.randn(batch_size, modal_dims[omics['cna']])  # 拷贝数变异数据
    ]

    downstream_output = model.forward(input_x=input_x, batch_size=batch_size, omics=omics)
    print(downstream_output.shape)
